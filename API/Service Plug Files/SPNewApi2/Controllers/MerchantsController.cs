...
namespace SPNewApi2.Controllers
{
    ...
    public class MerchantsController : ControllerBase
    {
        ...
        [HttpPut]
        [Route("editmerchant")]
        public async Task<ActionResult> userupdate([FromBody] Editmerch user)
        {
            ...
            var dbu = await _context.Merchants.FindAsync(userID);
            if (userID == null || userID <= 0)
            {
                return BadRequest("YOu are not log in");
            }
            ...
            dbu.MerchIdnumber = user.MerchIdnumber;
            dbu.MerchTaxnumber = user.MerchTaxnumber;
        ...
        //Get all reviews numbers
        [HttpGet]
        [Route("getallroofNO")]
        public async Task<IActionResult> getallrppfsnumber()
        {

            try
            {
                List<Merchant> listuser = _context.Merchants.Where(r => r.MerchType == "ROOFING").ToList();
                if (listuser != null)
                {
        ...
        [HttpGet]
        [Route("getallplumbNO")]
        public async Task<IActionResult> getallplumbsnumber()
        {

            try
            {
                List<Merchant> listuser = _context.Merchants.Where(r => r.MerchType == "PLUMBING").ToList();
                if (listuser != null)
                {
                    return Ok(listuser.Count);
                }
